...
def generate_question_prompt(words: List[str]) -> str:
    ...
    return f"""作為英文學測練習，我想要測試用戶是否學會這幾個單字：{words_str}，請你依照學測大考中心的宗旨和難度出題並提供答案和解析，每一個單字只需要出一題，並請確保正確選項ABCD是均勻分布的，選項必須符合台灣學測程度，而且不要把本次測驗其他題目要考的單字當作選項，每一題的每一個選項都應該獨一無二，請自己尋找具有誘答力的選項，但My ______, who is three years older than me, always helps me with my homework.(A)brother(B)sister(C)cousin(D)friend，這種題目就不行，因為有三個選項都選了都可以算對。
