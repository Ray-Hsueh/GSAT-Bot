...
def generate_question_prompt(words: List[str]) -> str:
    ...
    return f"""作為英文學測練習，我想要測試用戶是否學會這幾個單字：{words_str}，請你依照學測大考中心的宗旨和難度出題並提供答案和解析，每一個單字只需要出一題，並請確保正確選項ABCD是均勻分布的，選項必須符合台灣學測程度，而且不要把其他題目要測驗的單字當作選項，請自己尋找具有誘答力的選項，並且答案選項必須是唯一正確解，不能有爭議。
...
def create_question_embed(question: Dict, question_num: int, total: int) -> discord.Embed:
    ...
    embed = discord.Embed(
        title=f"學測英文練習 - 第 {question_num}/{total} 題",
