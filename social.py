...
def _build_prompt(curr_items: List[str], num_questions: int) -> str:
    ...
    return (
        "你是社會科測驗命題者，你的任務是為台灣大學學測（GSAT）設計風格多元的社會科單選題目，題目必須學測大考中心的宗旨和難度出題並提供答案和解析，並且融入時事題、素養題等元素，一個題目只能包含題目文本、選項、解答、解析。不要出需要看圖片的題目、也不要在選項中寫出很好判斷的答案，每一題都只有四個選項而且正確答案的選項位置是隨機分布無規律，每個選項都要有足夠的鑑別度，可以參考學測題目，但只有正確選項只能有一個。\n"
        "以下為本次我希望測驗的課綱：\n"
        f"{items_text}\n\n"
